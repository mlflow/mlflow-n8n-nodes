...
import { MlflowCallbackHandler } from './utils/MlflowCallbackHandler';
import { N8nLlmTracing } from './utils/N8nLlmTracing';

export class LmChatOpenAiMlflow implements INodeType {
	methods = {
		listSearch: {
			searchModels,
		},
	...
	description: INodeTypeDescription = {
		displayName: 'OpenAI Chat Model with MLflow',

		name: 'lmChatOpenAiMlflow',
		icon: { light: 'file:LmChatOpenAiMlflowLight.icon.svg', dark: 'file:LmChatOpenAiMlflowDark.icon.svg' },
		group: ['transform'],
		version: [1, 2, 3],
		description: 'For advanced usage with an AI chain',
		defaults: {
			name: 'OpenAI Chat Model with MLflow',
		},
		codex: {
			categories: ['AI'],
			subcategories: {
		...
		outputNames: ['Model'],
		credentials: [
			{ name: 'openAiApiWithMlflowApi', required: true },
		],
		requestDefaults: {
    ...
    "workflow": "main-flow"
						...
						description: 'Optional. Pass extra metadata to be attached to MLflow trace spans.',
					},
					{
						...
						description: 'Session identifier attached to the MLflow trace as mlflow.trace.session',
					},
					{
						displayName: 'User ID',
						name: 'userId',
						type: 'string',
						default: '',
						description: 'Optional user identifier attached to the MLflow trace as mlflow.trace.user',
					},
				],
			},
			{
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		const credentials = await this.getCredentials('openAiApiWithMlflowApi');

		const {
			sessionId,
			userId,
			customMetadata: customMetadataRaw = {},
		} = this.getNodeParameter('mlflowTracing', itemIndex) as {
			sessionId: string;
			userId?: string;
		...
		mlflow.init({
			trackingUri: credentials.mlflowTrackingUri as string,
			experimentId: credentials.mlflowExperimentId as string,
			...(credentials.mlflowTrackingToken
				? { trackingServerToken: credentials.mlflowTrackingToken as string }
				: {}),
		});

		const mlflowHandler = new MlflowCallbackHandler({ sessionId, userId, metadata: customMetadata });
		...
		const model = new ChatOpenAI({
			callbacks: [mlflowHandler, new N8nLlmTracing(this)],
